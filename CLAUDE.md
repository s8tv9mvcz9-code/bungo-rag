...
3. **Synesthesia layer** (`app/synesthesia.py`, on by default, `synesthesia=False` to skip): map the input text and each exemplar to Japanese traditional colors (lexicon → mood categories → Oklab-blended anchor colors → nearest 伝統色 name). Deterministic, no LLM, zero deps. Annotates each chunk with `color`/`color_name`, and produces a palette (input color / exemplar color / interpolated 連想 color). If the input has a strong enough mood signal, a one-line hint (`「藍鼠」——哀愁の氣配…`) is appended to the augmented prompt to subtly tint word choice — explicitly subordinate to conversion accuracy, and omitted entirely on weak signal so plain requests are never distorted.
4. Build messages = `SYSTEM_PROMPT` + last 8 history turns + an augmented user message that embeds the exemplars and orders the model to output converted text immediately (no preamble).
5. Stream tokens back. Returns `(token_generator, source_chunks, palette)` so the UI can render sources and the color band separately (`palette` is `None` when synesthesia is off).
...
- `app/synesthesia.py` — synesthesia layer (text → mood → Japanese traditional color). Pure functions, zero deps, deterministic: a curated lexicon (modern + historical-kana + 旧字 variants) scores 13 mood categories; anchor 伝統色 are blended in Oklab space and named by nearest-neighbor against a ~47-color table. Weak signal ⇒ neutral 白磁 + no prompt hint (practicality guard). Tested in `eval/test_synesthesia.py`; wiring tested in `backend/test_contract.py`.
- `backend/main.py` — FastAPI wrapper around `app/rag.py` for the native apps. `POST /chat` streams NDJSON events (`token` / `sources` / `done` / `error`); `GET /health` returns `{"status":"ok","version":"<BUILD_SHA>"}` where `BUILD_SHA` is injected by the deploy workflow (SSOT traceability). The event/field contract is mirrored in `ios/Sources/Model/Models.swift` and `android/.../data/Models.kt` — all three are verified consistent (extra keys like `book_id` are safely ignored by both clients; Android sets `ignoreUnknownKeys`, but **unknown event `type`s still throw** there — make clients forward-compatible before adding new event types). **The synesthesia palette rides on the `sources` event as an extra `palette` key** (plus `color`/`color_name` on each source) precisely because of that constraint — old clients ignore the keys, new clients render the color band; request field `synesthesia` (optional, default `true`) toggles it. Event order (`token*` → `sources` → `done`) is unchanged. Contract regression-tested in `backend/test_contract.py` (run by `backend-ci.yml`).

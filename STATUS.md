...
8. **共感覚レイヤー（情緒の模倣・実験）** — 文 → 情調 → **日本の伝統色**（`app/synesthesia.py`、純関数・LLM不使用・決定的）。入力文の色と手本の色を Oklab で補間した「連想色」を3クライアントに表示（Web=グラデ帯＋出典チップ、iOS/Android=帯＋色点）。強信号時のみ生成プロンプトに一行の情調ヒントを注入（**意味の正確な変換が常に最優先**、弱信号では完全不介入=実用性ガード）。API は `sources` イベントへの追加キーのみで**完全後方互換**（Android の未知イベント型 throw 制約を回避）。`eval/test_synesthesia.py`＋`backend/test_contract.py` で机上検証済み。**生成への実効果はモデル復旧後に要確認**。

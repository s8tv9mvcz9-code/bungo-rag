...
**Abuse / cost guards on the public API (`backend/main.py`).** `/chat` is unauthenticated and `--ingress external`, and the model is high-cost `claude-opus-4-8`, so the backend clamps per-request amplification and rate-limits volume. All knobs are optional env vars (defaults in parens): `MAX_MESSAGE_CHARS` (4000), `MAX_CONTENT_CHARS` (4000, per history item), `MAX_HISTORY_ITEMS` (16), `MAX_TOP_K` (10, matches the web slider's max) → enforced as pydantic `Field` limits (over-limit ⇒ 422); `RATE_MAX_PER_IP` (20) per `RATE_WINDOW_SEC` (60s) and a global `DAILY_REQUEST_CAP` (1000/24h) → over-limit ⇒ 429. **Caveats:** the rate limiter is in-memory (per-replica, *lost on scale-to-zero*) and `X-Forwarded-For` is spoofable, so per-IP is best-effort — the spoof-resistant backstop is the global daily cap, and the only true $/hour ceiling is the **Foundry deployment's TPM quota (set it in Azure)**. Malformed requests also count against the rate limit (intentional). Clients surface a 429/422 as a generic error (Android decodes the non-NDJSON error body and throws; iOS throws `badServerResponse`).
